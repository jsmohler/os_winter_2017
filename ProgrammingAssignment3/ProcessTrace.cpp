...
#include "QuotaExceededException.h"
#include <algorithm>
#include <cctype>
#include <iomanip>
#include <iostream>
...
void ProcessTrace::Execute(uint32_t num) {
  ...
  if (ParseCommand(line, cmd, cmdArgs)) {
    if (cmd == "compare") {
      ...
      CmdQuota(line, cmd, cmdArgs); // set max number of usable pages
    } else {
...
void ProcessTrace::CmdAlloc(const vector<uint32_t> &cmdArgs) {
  // Get arguments
...
void ProcessTrace::CmdPut(const string &line,
  ...
  try {
    ...
    memory.put_bytes(addr, num_bytes, buffer);
      ...
      mem::PMCB current_state;
      memory.get_PMCB(current_state);
      try {
        std::vector<uint32_t> args = {(current_state.next_vaddress/kPageSize)*kPageSize, kPageSize};
        CmdAlloc(args);
...
void ProcessTrace::CmdCopy(const string &line,
  ...
  if(bytes_read != 0) {
    try {
      ...
      memory.get_PMCB(current_state);
      try {
        std::vector<uint32_t> args = {(current_state.next_vaddress/0x1000)*kPageSize, 0x1000};
        CmdAlloc(args);
        memory.set_PMCB(current_state);
        num_allocated++;
      } catch (QuotaExceededException a) {
        current_state.operation_state = mem::PMCB::NONE;
        memory.set_PMCB(current_state);
      }
    } catch(WritePermissionFaultException e) {
  ...
  try {
    for(int i = 0; i < num_bytes; ++i) {
      ...
      try {
        ...
        memory.set_PMCB(current_state);
        num_allocated++;
        std::vector<uint32_t> restart = {current_state.next_vaddress, remaining_bytes, cmdArgs.at(2)};
        CmdFill("", "", restart);
      } catch (QuotaExceededException a) {
        current_state.operation_state = mem::PMCB::NONE;
        memory.set_PMCB(current_state);
